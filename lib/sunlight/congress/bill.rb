...
class Sunlight::Congress
  def bill
    Sunlight::Congress::Bill::Finder.new(self.api_key)
  end
  class Bill
    ...
    class Finder
      ...
      def by_title(title)
        uri = URI("#{Sunlight::Congress::BASE_URI}/bills/search?query=#{title}&apikey=#{api_key}")

        JSON.load(Net::HTTP.get(uri))["results"].collect do |json|
          Sunlight::Congress::Bill.new(json)
        end
      end

      def by_congress(congress)
        uri = URI("http://congress.api.sunlightfoundation.com/bills?congress=#{congress}&apikey=#{api_key}")
      ...
      def by_term(term)
        uri = URI("http://congress.api.sunlightfoundation.com/bills/search?query='#{term}'&apikey=#{api_key}")
      ...
      def by_sponsor_party(party)
        party = party[0].upcase
        uri = URI("http://congress.api.sunlightfoundation.com/bills?sponsor.party='#{party}'&apikey=#{api_key}")
      ...
      def by_bill_id(bill_id)
        uri = URI("#{Sunlight::Congress::BASE_URI}/bills?bill_id=#{bill_id}&apikey=#{api_key}")

        Sunlight::Congress::Bill.new(JSON.load(Net::HTTP.get(uri))['results'].first)
      end
    end
  end
end
